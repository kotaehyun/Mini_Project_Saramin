...
NODE_CONTRACTS = {
    "START": {
        "module": "run_job_search_workflow*",
        "role": "사용자 요청을 LangGraph 상태로 초기화",
        "inputs": [
            "keyword: str",
    ...
    "검색 URL 생성": {
        "module": "crawlers.saramin_crawler.generate_search_url",
        "role": "검색어와 페이지 번호를 사람인 목록 URL로 변환",
        "inputs": [
            "keyword: str",
            "page: int",
        ],
        ...
        "writes": [
            "HTTP request target URL: str",
        ],
    },
    "사람인 공고 크롤링": {
        "module": "graphs.job_graph.crawl_jobs_node",
        "role": "사람인 목록 응답을 공고 딕셔너리 목록으로 추출",
        "inputs": [
            "state: JobSearchState",
            "state.keyword: str",
        ],
        "outputs": [
            "raw_jobs: List[Dict[str, Any]]",
            "raw_jobs[i]: JobPosting-like dict",
        ],
        "reads": [
            "keyword: str",
            "search_url: str per page",
        ],
        "writes": [
            "state.raw_jobs: List[Dict[str, Any]]",
            "state.error: str",
        ],
    },
    "공고 데이터 전처리": {
        "module": "graphs.job_graph.preprocess_jobs_node",
        "role": "공고를 정제하고 LLM용 설명 문자열을 추가",
        "inputs": [
            "raw_jobs: List[Dict[str, Any]]",
        ],
        "outputs": [
            "processed_jobs: List[Dict[str, Any]]",
            "processed_jobs[i].job_description: str",
        ],
        "reads": [
            "state.raw_jobs: List[Dict[str, Any]]",
        ],
        "writes": [
            "state.processed_jobs: List[Dict[str, Any]]",
            "state.error: str",
        ],
    },
    "LLM 적합도 평가": {
        "module": "graphs.job_graph.filter_jobs_node",
        "role": "프로필과 공고를 비교해 공고별 평가 JSON 생성",
        "inputs": [
            "user_profile: Dict[str, Any]",
            "processed_jobs: List[Dict[str, Any]]",
        ],
        "outputs": [
            "evaluation_results: List[Dict[str, Any]]",
            "evaluation_results[i]: EvaluationResult-like dict",
        ],
        "reads": [
        ...
        "writes": [
            "state.evaluation_results: List[Dict[str, Any]]",
            "state.error: str",
        ],
    },
    "결과 요약 생성": {
        "module": "graphs.job_graph.summarize_result_node",
        "role": "평가 결과 빈도와 추천 요약을 생성",
        "inputs": [
            "user_profile: Dict[str, Any]",
            "evaluation_results: List[Dict[str, Any]]",
        ],
        ...
        "writes": [
            "state.summary: Dict[str, Any]",
            "state.error: str",
        ],
    },
    "결과 파일 저장": {
        "module": "utils.output_formatter + Streamlit caller",
        "role": "평가 산출물을 다운로드 가능한 파일로 저장",
        "inputs": [
            "evaluation_results: List[Dict[str, Any]]",
            "summary: Dict[str, Any]",
            "user_profile: Dict[str, Any]",
    ...
    "END": {
        "module": "LangGraph END / UI result surface",
        "role": "최종 상태와 저장 파일을 결과 화면으로 전달",
        "inputs": [
            "final_state: JobSearchState",
            "saved_paths: Dict[str, str] in caller",
        ],
        "outputs": [
            "dashboard tables",
            "download files",
        ...
        "reads": [
            "raw_jobs / processed_jobs",
            "evaluation_results / summary",
        ],
        "writes": [
            "No further JobSearchState mutation",
        ],
    },
}
...
def render_langgraph_flow(status=None):
    ...
    <div id="space-flow-root">
      ...
      <section id="space-flow-type-map" aria-label="LangGraph data contract flow">
        <div class="type-map-head">
          <span>Data contract flow</span>
          <small>node input type -> state mutation -> output type</small>
        </div>
        <div id="space-flow-contract-rail"></div>
      </section>
    </div>
    ...
    <style>
      #space-flow-root {{
        ...
        border-radius: 8px;
        background: 
          radial-gradient(circle at 15% 40%, rgba(138, 43, 226, 0.12) 0%, transparent 50%),
          radial-gradient(circle at 85% 70%, rgba(30, 144, 255, 0.1) 0%, transparent 50%),
      ...
      @keyframes nebulaShift {{
        ...
        50% {{ transform: translate(4%, 3%) scale(1.03); opacity: 0.34; }}
        100% {{ transform: translate(-3%, -1%) scale(1); opacity: 0.28; }}
      }}
      #space-flow-fallback {{
        position: absolute;
        inset: 0;
      ...
      .contract-link {{
        ...
        flex: 0 0 34px;
        color: #a0c3ec;
        font: 400 20px "Geist Mono", "JetBrains Mono", Consolas, monospace;
      }}
      .contract-card {{
        width: 270px;
        min-height: 0;
      ...
      #space-flow-info b {{
        color: #a0a8af;
        font: 400 10px "Geist Mono", "JetBrains Mono", Consolas, monospace;
        letter-spacing: 1px;
        text-transform: uppercase;
      }}
      .contract-io code {{
    ...
    <script type="module">
      ...
      const contractRail = document.getElementById("space-flow-contract-rail");
      ...
      function contractList(title, values) {{
        return `
          <div class="info-list">
      ...
      function updateContractSelection(order) {{
        selectedOrder = order;
        contractRail.querySelectorAll(".contract-card").forEach((card) => {{
          card.classList.toggle("is-selected", Number(card.dataset.order) === order);
        }});
      }}
      function showContract(node, themeName = "") {{
        const contract = node.contract;
        updateContractSelection(node.order);
        info.innerHTML = `
          <div class="info-title">
            <strong>${{node.order + 1}}. ${{escapeHtml(node.name)}}</strong>
            <span>${{escapeHtml(contract.module)}}${{themeName ? ` · ${{escapeHtml(themeName)}}` : ""}}</span>
          </div>
          <p class="info-role">${{escapeHtml(contract.role)}} · 상태 ${{escapeHtml(node.status)}}</p>
          <div class="info-grid">
            ${{contractList("Input type", contract.inputs)}}
            ${{contractList("Output type", contract.outputs)}}
            ${{contractList("State reads", contract.reads)}}
            ${{contractList("State writes", contract.writes)}}
          </div>
        `;
      }}
      function compactIo(title, values) {{
        return `
          <div class="contract-io">
            <b>${{escapeHtml(title)}}</b>
            ${{values.slice(0, 2).map((value) => `<code>${{escapeHtml(value)}}</code>`).join("")}}
          </div>
        `;
      }}
      function renderContractRail() {{
        contractRail.innerHTML = nodeData.map((node, index) => `
          <div class="contract-step">
            <article class="contract-card" data-order="${{node.order}}">
              <div class="contract-node">
                <span>${{escapeHtml(node.label)}}</span>
                <em>${{escapeHtml(node.status)}}</em>
              </div>
              ${{compactIo("Input", node.contract.inputs)}}
              ${{compactIo("Output", node.contract.outputs)}}
            </article>
            ${{index < nodeData.length - 1 ? `<span class="contract-link">-></span>` : ""}}
          </div>
        `).join("");
      }}
      renderContractRail();
      contractRail.addEventListener("click", (event) => {{
        const card = event.target.closest(".contract-card");
        if (!card) return;
        const node = nodeData.find((item) => item.order === Number(card.dataset.order));
        if (node) showContract(node);
      }});
      showContract(nodeData[0]);
    ...
    # convert doubled braces (used to escape f-string) back to single braces
    html = html.replace("{{", "{").replace("}}", "}")
    # inject JSON payloads
    html = html.replace("{nodes_json}", nodes_json).replace("{progress_json}", progress_json)
    ...
    st.iframe(html_path, height=840, width="stretch")
